...
func (s *SnapshotStore) Get(id int64) (*model.Snapshot, error) {
	...
	s.overlayLiveJumps(snap)
	return snap, nil
}

func (s *SnapshotStore) overlayLiveJumps(snap *model.Snapshot) {
	if snap == nil {
		return
	}
	rows, err := s.db.Query(
		`SELECT id, magnitude_ppb, direction, status FROM jump_segments WHERE clock_id=? ORDER BY id`,
		snap.ClockID)
	if err != nil {
		return
	}
	defer rows.Close()
	var frozen []model.FrozenJump
	for rows.Next() {
		var fj model.FrozenJump
		if err := rows.Scan(&fj.ID, &fj.MagnitudePPB, &fj.Direction, &fj.Status); err != nil {
			return
		}
		frozen = append(frozen, fj)
	}
	if snap.Evidence == nil {
		snap.Evidence = &model.FrozenEvidence{}
	}
	snap.Evidence.Jumps = frozen
}
